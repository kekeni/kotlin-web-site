...
By default an Objective-C framework produced by Kotlin/Native supports only one platform. But such frameworks can be merged
into a single universal (fat) binary using the `lipo` utility. E.g. such an operation makes sense for 32-bit and 64-bit iOS
frameworks. In this case the resulting universal framework can be used on both 32-bit and 64-bit devices.

The Gradle plugin provides a separate task which creates a universal framework for iOS targets from several regular ones.
